...
module.exports = (argFormats = {}) => {
    ...
    const parseFor = (name) => {
        if (parsers[name] !== undefined) {
            ...
            return parsers[name].map((parser) => parser(args.shift()));
        }
        return true;
    };
    ...
    while (args.length > 0) {
        ...
        switch (true) {
            case arg.startsWith("--") === true: {
                ...
                results[
                    (nullref0 = names[argName]) != null ? nullref0 : argName
                ] = parseFor(argName);
                break;
            }
            case arg.startsWith("-") === true: {
                ...
                for (const flag of flags) {
                    results[
                        (nullref1 = names[flag]) != null ? nullref1 : flag
                    ] = parseFor(flag);
                }
                break;
            }
